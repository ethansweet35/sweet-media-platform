 ...
 *
 * Usage:
 *   <RankingKeywordsPopover pageUrl="https://example.com/about">
...
// Module-level cache — persists across renders within the same browser session.
...
export interface RankingKeywordsPopoverProps {
  /** Full URL of the page to look up (e.g. "https://example.com/about"). */
  pageUrl: string;
  children: ReactNode;
}
...
export default function RankingKeywordsPopover({
  ...
  return (
    ...
    >
      ...
      {open && (
        <div
          className="absolute left-0 top-full z-50 mt-1.5 w-72 rounded-xl border border-neutral-200 bg-white shadow-lg"
          onMouseEnter={handleMouseEnter}
          onMouseLeave={handleMouseLeave}
        >
          {/* Arrow */}
              ...
              <ul className="divide-y divide-neutral-50">
                {rows.map((row, i) => (
                  <li key={row.query} className="flex items-center gap-2.5 px-2 py-2">
                    ...
                    <span
                      className={`shrink-0 inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold ${positionBadgeClass(Math.round(row.position))}`}
                    >
