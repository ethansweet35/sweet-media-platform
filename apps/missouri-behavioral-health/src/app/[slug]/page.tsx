...
const SITE_URL = process.env.NEXT_PUBLIC_SITE_URL || "https://missouribehavioralhealth.com";
const BRAND_NAME = "Missouri Behavioral Health";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = `${titleBase} | ${BRAND_NAME}`;
  const description =
    metaString(row.meta_description).trim() ||
    metaString(row.excerpt).trim() ||
    "Mental health and addiction recovery resources from Missouri Behavioral Health.";
...
function BlogPostLoadingShell() {
  return (
    <div className="min-h-screen bg-cream">
      <div className="mx-auto max-w-7xl px-6 pt-36 pb-20 animate-pulse">
        <div className="mb-8 h-4 w-1/4 rounded bg-mbh-green/10" />
        <div className="mb-6 h-12 w-3/4 rounded bg-mbh-green/10" />
        <div className="mb-10 h-4 w-1/3 rounded bg-mbh-green/10" />
        <div className="h-[320px] w-full rounded-2xl bg-mbh-green/10 md:h-[460px]" />
      </div>
    </div>
  );
}
