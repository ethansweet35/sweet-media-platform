...
const trustBullets = [
  "Per diem HCPCS H0017, H0018, and H0019 for behavioral health residential programs",
  "ASAM level-of-care documentation and place of service 55 for mental health & substance use facilities",
  "New authorization on every level-of-care change—including PHP step-down",
  ...cipherHomeMetricBullets.slice(0, 2),
] as const;

export default function ResidentialLandingHero() {
  return (
    <section className={`relative overflow-hidden bg-[#101E3F] ${LANDING_SECTION_PY}`}>
      <div className="absolute inset-0">
        <Image
          src={RESIDENTIAL_LANDING_IMAGES.hero}
          alt="Residential mental health and addiction treatment facility for behavioral health billing services"
          fill
          priority
        ...
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1.08fr)_minmax(0,0.92fr)] lg:items-center lg:gap-14">
          <div className="max-w-xl">
            ...
            <h1 className="mt-5 font-[var(--font-heading)] text-4xl font-medium leading-[1.08] tracking-[-0.02em] text-white md:text-5xl lg:text-[3.25rem]">
              Residential treatment billing services that protect{" "}
              <span className="text-[#5eb5e0]">your census revenue</span>
            </h1>

            <p className="mt-6 text-[15px] leading-[1.75] text-white/85 md:text-base">
              Cipher&apos;s residential treatment billing services help mental health and addiction treatment centers bill
              inpatient programs with clean claims—Medicaid, Medicare, and commercial coverage—so staff are not buried in
              authorization lapses, ASAM documentation gaps, and claim denials.
            </p>
          ...
          >
            ...
            <h2 className="mt-2 font-[var(--font-heading)] text-2xl font-medium leading-snug text-white md:text-[1.65rem]">
              Request your residential billing review
            </h2>
            <p className="mt-2 text-[13px] leading-relaxed text-white/55">
              Tell us about your treatment facility. We&apos;ll review your residential treatment billing setup against
              payer requirements—so you see where documentation or coding may be leaving reimbursement on the table.
            </p>
            <div className="mt-6">
              <ServiceLandingLeadForm programLabel="Residential Treatment Billing Services" />
            </div>
          </div>
        </div>
      </div>
