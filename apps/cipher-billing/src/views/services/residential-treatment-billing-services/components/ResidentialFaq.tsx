...
import { RESIDENTIAL_FAQ_ITEMS } from "@/views/services/residential-treatment-billing-services/residentialFaqItems";
import { LANDING_SECTION_PY, SERVICE_CONTAINER } from "@/views/services/components/servicePageConstants";

const faqSchema = buildFAQPageSchema(RESIDENTIAL_FAQ_ITEMS);

export default function ResidentialFaq() {
  return (
    <section id="residential-faq" className={`bg-[#F5F7FA] ${LANDING_SECTION_PY}`}>
        ...
        <div className="grid gap-10 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)] lg:items-start lg:gap-14">
          <div>
            ...
            <h2 className="mt-4 font-[var(--font-heading)] text-[1.75rem] font-medium leading-tight text-[#101E3F] md:text-[2.125rem]">
              Residential billing frequently asked <span className="text-[#166C96]">questions</span>
            </h2>
            <p className="mt-4 max-w-md text-[15px] leading-[1.65] text-slate-600">
              Six common questions from residential treatment operators—codes, coverage, compliance, and when to call
              Cipher.
            </p>
            <p className="mt-4 max-w-md text-[14px] leading-[1.65] text-slate-500">
              More detail on LOC transitions, ASAM, and therapy bundling is in{" "}
              <a
                href="#residential-billing-essentials"
            ...
            <Link
              href="/contact-us?source=residential-billing-faq"
            ...
            >
              Talk to a residential billing specialist
              <i className="ri-arrow-right-line text-base" aria-hidden />
            </Link>
          </div>

          <div className="grid gap-3">
            {RESIDENTIAL_FAQ_ITEMS.map((item) => (
              <details
                key={item.question}
