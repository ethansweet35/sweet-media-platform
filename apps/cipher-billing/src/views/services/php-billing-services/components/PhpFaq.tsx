...
import { PHP_FAQ_ITEMS } from "@/views/services/php-billing-services/phpFaqItems";
import { LANDING_SECTION_PY, SERVICE_CONTAINER } from "@/views/services/components/servicePageConstants";

const faqSchema = buildFAQPageSchema(PHP_FAQ_ITEMS);

export default function PhpFaq() {
  return (
    <section id="php-faq" className={`bg-[#F5F7FA] ${LANDING_SECTION_PY}`}>
        ...
        <div className="grid gap-10 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)] lg:items-start lg:gap-14">
          <div>
            ...
            <h2 className="mt-4 font-[var(--font-heading)] text-[1.75rem] font-medium leading-tight text-[#101E3F] md:text-[2.125rem]">
              PHP billing frequently asked <span className="text-[#166C96]">questions</span>
            </h2>
            ...
            <p className="mt-4 max-w-md text-[14px] leading-[1.65] text-slate-500">
              More detail on inpatient vs PHP, documentation, and payer rules is in{" "}
              <a
                href="#php-billing-essentials"
            ...
            <Link
              href="/contact-us?source=php-billing-faq"
