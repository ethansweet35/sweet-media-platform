...
import { PHP_BILLING_ESSENTIALS } from "@/views/services/php-billing-services/phpBillingEssentials";

export default function PhpBillingEssentials() {
  return (
    <section id="php-billing-essentials" className={`bg-[#F5F7FA] ${SERVICE_SECTION_PY_COMPACT}`}>
      <div className={SERVICE_CONTAINER}>
        <div className="mx-auto max-w-3xl text-center">
          <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-[#166C96]">PHP billing essentials</p>
          <h2 className="mt-3 font-[var(--font-heading)] text-[1.75rem] font-medium leading-tight text-[#101E3F] md:text-[2rem]">
            What operators need to know <span className="text-[#166C96]">before claims go out</span>
          </h2>
          <p className="mt-3 text-[14px] leading-[1.6] text-slate-600">
            The details that drive denials—without an endless FAQ list. For code crosswalks see{" "}
            <a href="#php-billing-codes" className="font-semibold text-[#166C96] underline-offset-2 hover:underline">
              PHP billing codes
            </a>
            ; for hour and certification rules see{" "}
            <a
              href="#php-billing-requirements"
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:gap-5">
          {PHP_BILLING_ESSENTIALS.map((item) => (
            <article
              key={item.title}
