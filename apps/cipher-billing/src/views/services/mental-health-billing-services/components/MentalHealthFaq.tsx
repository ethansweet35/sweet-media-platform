...
import { MENTAL_HEALTH_FAQ_ITEMS } from "@/views/services/mental-health-billing-services/mentalHealthFaqItems";
import { LANDING_SECTION_PY, SERVICE_CONTAINER } from "@/views/services/components/servicePageConstants";

const faqSchema = buildFAQPageSchema(MENTAL_HEALTH_FAQ_ITEMS);

export default function MentalHealthFaq() {
  return (
    <section id="mental-health-faq" className={`bg-[#F5F7FA] ${LANDING_SECTION_PY}`}>
        ...
        <div className="grid gap-10 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)] lg:items-start lg:gap-14">
          <div>
            ...
            <h2 className="mt-4 font-[var(--font-heading)] text-[1.75rem] font-medium leading-tight text-[#101E3F] md:text-[2.125rem]">
              Mental health billing <span className="text-[#166C96]">questions</span>
            </h2>
            <p className="mt-4 max-w-md text-[15px] leading-[1.65] text-slate-600">
              Six common questions from therapy and psychiatric practices—coverage, denials, credentialing, and when to
              call Cipher.
            </p>
            <p className="mt-4 max-w-md text-[14px] leading-[1.65] text-slate-500">
              Want more information on telehealth or dual diagnosis? See{" "}
              <a
                href="#mental-health-billing-essentials"
            ...
            <Link
              href="/contact-us?source=mental-health-billing-faq"
          ...
          <div className="grid gap-3">
            {MENTAL_HEALTH_FAQ_ITEMS.map((item) => (
              <details
                key={item.question}
