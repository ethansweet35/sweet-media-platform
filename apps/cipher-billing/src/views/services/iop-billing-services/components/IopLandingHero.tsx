...
const IMG_HERO = IOP_LANDING_IMAGES.hero;

const trustBullets = [
  "Medicare coverage for intensive outpatient & condition code 92",
  "HCPCS and CPT billing codes aligned to your outpatient program",
  "IOP services billing—auth, denials, and PHP overlap prevention",
  ...cipherHomeMetricBullets.slice(0, 2),
] as const;

export default function IopLandingHero() {
  return (
    <section className={`relative overflow-hidden bg-[#101E3F] ${LANDING_SECTION_PY}`}>
      <div className="absolute inset-0">
        <Image
          src={IMG_HERO}
          alt="Behavioral health team coordinating intensive outpatient program billing for mental health and substance use treatment"
          fill
          priority
        ...
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1.08fr)_minmax(0,0.92fr)] lg:items-center lg:gap-14">
          <div className="max-w-xl">
            ...
            <h1 className="mt-5 font-[var(--font-heading)] text-4xl font-medium leading-[1.08] tracking-[-0.02em] text-white md:text-5xl lg:text-[3.25rem]">
              IOP billing services that keep{" "}
              <span className="text-[#5eb5e0]">revenue moving</span>
            </h1>

            <p className="mt-6 text-[15px] leading-[1.75] text-white/85 md:text-base">
              Cipher&apos;s IOP billing services help mental health and substance use providers run intensive outpatient
              programs with clean claims—Medicare coverage, Medicaid, and commercial payers—so your team is not buried in
              denials, auth lapses, and rework on IOP services.
            </p>
          ...
          >
            ...
            <h2 className="mt-2 font-[var(--font-heading)] text-2xl font-medium leading-snug text-white md:text-[1.65rem]">
              Request your IOP billing review
            </h2>
            <p className="mt-2 text-[13px] leading-relaxed text-white/55">
              Tell us about your outpatient program. We&apos;ll review your IOP billing setup and IOP billing requirements
              against payer rules—so you see where claims or documentation may be leaving money on the table.
            </p>
            <div className="mt-6">
              <ServiceLandingLeadForm programLabel="IOP Billing Services" />
            </div>
          </div>
        </div>
      </div>
