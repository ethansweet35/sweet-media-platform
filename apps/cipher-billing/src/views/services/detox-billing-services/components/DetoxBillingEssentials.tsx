...
import { DETOX_BILLING_ESSENTIALS } from "@/views/services/detox-billing-services/detoxBillingEssentials";

export default function DetoxBillingEssentials() {
  return (
    <section id="detox-billing-essentials" className={`bg-[#F5F7FA] ${SERVICE_SECTION_PY_COMPACT}`}>
      <div className={SERVICE_CONTAINER}>
        <div className="mx-auto max-w-3xl text-center">
          ...
          <h2 className="mt-3 font-[var(--font-heading)] text-[1.75rem] font-medium leading-tight text-[#101E3F] md:text-[2rem]">
            What operators need to <span className="text-[#166C96]">know first</span>
          </h2>
          <p className="mt-3 text-[14px] leading-[1.6] text-slate-600">
            Program detail without a long FAQ—see{" "}
            <a href="#detox-billing-codes" className="font-semibold text-[#166C96] underline-offset-2 hover:underline">
              HCPCS codes
            </a>{" "}
            for the full H0008–H0014 crosswalk.
          </p>
        </div>

        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:gap-5">
          {DETOX_BILLING_ESSENTIALS.map((item) => (
            <article
              key={item.title}
