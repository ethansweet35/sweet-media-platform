...
--
-- Apply this to every brand's Supabase project. Idempotent (uses if not exists).

-- =========================================================
-- BLOG POSTS — Surfer columns
-- =========================================================
...
-- Notes:
-- * surfer_audit_state values: 'scheduled' | 'completed' | 'error'.
-- * surfer_content_score: 0–100 (Surfer's content-score scale).
-- * surfer_guidance_applied: writer-set flag indicating the post/page has been
--   rewritten using the linked Surfer Content Editor's recommendations.
-- * published_url: optional override URL sent to Surfer Audit (overrides the
--   default `${NEXT_PUBLIC_SITE_URL}${NEXT_PUBLIC_BLOG_PATH_BASE}/${slug}` for
--   blog_posts, and `${NEXT_PUBLIC_SITE_URL}${route_path}` for tracked_pages).
