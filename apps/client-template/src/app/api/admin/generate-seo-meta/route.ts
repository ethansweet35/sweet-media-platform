...
function buildPrompt(body: GenerateSeoMetaRequest): string {
  ...
  if (isPost) {
    ...
    return `You are an expert SEO copywriter for a company (Client Brand). Generate an optimized meta description for a blog post.
...
- Return ONLY valid JSON, no markdown fences: {"seo_title":null,"meta_description":"..."}`;
  ...
  return `You are an expert SEO copywriter for a company (Client Brand). Generate an SEO title and meta description for the following page.
...
Rules:
- SEO title: 50–60 characters, compelling, includes the keyword naturally, includes the brand name if space allows.
