...
import type { ConditionTreatmentConfig } from "@/lib/condition-treatment-pages";
import { CONDITION_IMGS, CONTAINER, SITE } from "@/lib/site";

type SchoolAvoidanceTreatmentPageProps = {
  config: ConditionTreatmentConfig;
};

const schoolAvoidanceStats = [
  { value: "Exposure", label: "based re-entry planning in IOP", icon: "ri-route-line" },
  { value: "9–20", label: "clinical hours per week in Virtual IOP", icon: "ri-time-line" },
  { value: "School", label: "coordination when families authorize", icon: "ri-building-line" },
  { value: "24–48", label: "hours typical intake timeline", icon: "ri-calendar-check-line" },
];

const schoolAvoidanceProfiles = [
  {
    icon: "ri-sun-line",
    title: "Morning panic",
    body: "Meltdowns, crying, or physical complaints before school — mornings become a daily battle.",
  },
  {
    icon: "ri-calendar-close-line",
    title: "Absences & tardiness",
    body: "Repeated missed days, leaving early, or chronic lateness tied to distress rather than disinterest.",
  },
  {
    icon: "ri-heart-pulse-line",
    title: "Physical symptoms",
    body: "Headaches, stomachaches, or nausea that appear on school days and ease on weekends.",
  },
  {
    icon: "ri-book-open-line",
    title: "Academic slip",
    body: "Grades fall and assignments pile up — often despite real capability when avoidance started.",
  },
];

const avoidanceCycleSteps = [
  { label: "Trigger", detail: "School morning or specific class", tone: "text-white/70" },
  { label: "Spike", detail: "Anxiety, panic, or dread", tone: "text-accent" },
  { label: "Avoid", detail: "Stay home, leave early, or disengage", tone: "text-white/70" },
  { label: "Relief", detail: "Short-term drop in anxiety", tone: "text-white/50" },
  { label: "Loop", detail: "School feels more dangerous", tone: "text-white/90" },
];

const reEntrySteps = [
  { level: "1", task: "Map triggers and build coping scripts for morning transitions at home", progress: 20 },
  { level: "2", task: "Practice arriving near school or entering building for brief periods", progress: 40 },
  { level: "3", task: "Attend one class or partial day with clinician-supported plan", progress: 55 },
  { level: "4", task: "Extend attendance with exposure homework between IOP sessions", progress: 75 },
  { level: "5", task: "Maintain full schedule with family and school support structures", progress: 100 },
];

const familySchoolShifts = [
  { before: "Negotiating every morning until teen wins", after: "Consistent routines with clinician-guided responses" },
  { before: "Homeschooling indefinitely without treatment", after: "Clinical treatment plus paced re-entry timeline" },
  { before: "Forcing full attendance before skills are in place", after: "Gradual exposure ladder matched to capacity" },
  { before: "School and therapist not communicating", after: "Shared re-entry plan with family authorization" },
];

const underlyingDrivers = [
  { icon: "ri-heart-pulse-line", label: "Anxiety disorders", detail: "Social anxiety, GAD, and panic often drive school refusal — treated with CBT and exposure.", path: "/online-anxiety-treatment" },
  { icon: "ri-emotion-sad-line", label: "Depression", detail: "Withdrawal and low motivation present as avoidance — behavioral activation integrated in IOP.", path: "/teen-depression-treatment" },
  { icon: "ri-focus-3-line", label: "ADHD & executive function", detail: "Academic frustration and disorganization can trigger refusal patterns.", path: "/adhd-treatment-for-teens" },
  { icon: "ri-shield-flash-line", label: "Trauma", detail: "School environments may feel unsafe after adverse experiences — trauma-informed pacing.", path: "/ptsd-treatment-online" },
];
...
const schoolSupports = [
  ...
  "Morning routine and transition strategies for refusal patterns",
  "Teacher scripts that reinforce skills — not accommodation that maintains avoidance",
];

export default function SchoolAvoidanceTreatmentPage({ config }: SchoolAvoidanceTreatmentPageProps) {
  const heroSrc = CONDITION_IMGS[config.imageKey];

  return (
    <MarketingPage currentPath="/conditions/school-avoidance">
      <ProgramHeroSection
      ...
      <section className="bg-surface px-6 py-section lg:px-10">
        <div className={CONTAINER}>
          <div className="mb-12 max-w-2xl">
            <p className="mb-3 text-[11px] font-bold uppercase tracking-[0.3em] text-accent"><AutoLinkedText>{"School avoidance in adolescence"}</AutoLinkedText></p>
            <h2 className="text-4xl font-bold text-ink md:text-5xl" style={{ fontFamily: "var(--font-heebo)" }}>
              When staying home feels safer than going to school
            </h2>
            <p className="mt-4 text-sm leading-8 text-body">
              <AutoLinkedText>{"School refusal is rarely about not caring — it is usually anxiety, mood, or trauma driving avoidance.\n              Virtual IOP treats those drivers while building a re-entry plan that families and schools can follow."}</AutoLinkedText>
            </p>
          </div>

          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {schoolAvoidanceProfiles.map((item) => (
              <div
                key={item.title}
      ...
      <section className="bg-white px-6 py-16 lg:px-10 lg:py-20">
        <div className={CONTAINER}>
          <div className="max-w-3xl">
            <p className="mb-3 text-[11px] font-bold uppercase tracking-[0.3em] text-accent"><AutoLinkedText>{"Why avoidance persists"}</AutoLinkedText></p>
            <h2 className="text-3xl font-bold text-ink md:text-4xl lg:text-[2.75rem]" style={{ fontFamily: "var(--font-heebo)" }}>
              The school avoidance loop — and where exposure breaks it
            </h2>
            <p className="mt-4 text-sm leading-8 text-body">
              <AutoLinkedText>{"Staying home reduces anxiety in the short term — which teaches the brain that school is dangerous. Weekly\n              therapy often cannot provide enough exposure coaching between school days. Virtual IOP adds frequent\n              skills practice and family alignment for gradual re-entry."}</AutoLinkedText>
            </p>
          </div>
          ...
          <div className="mt-10 overflow-hidden rounded-3xl bg-dark ring-1 ring-white/10">
            <div className="flex divide-x divide-white/10 overflow-x-auto lg:grid lg:grid-cols-5 lg:overflow-visible">
              {avoidanceCycleSteps.map((step, i) => (
                <div
                  key={step.label}
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    {i < avoidanceCycleSteps.length - 1 ? (
            ...
            <div className="flex flex-col gap-4 border-t border-white/10 bg-accent/[0.08] px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:gap-8 lg:px-8">
              <div className="flex items-start gap-3 sm:min-w-0 sm:flex-1">
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-accent/20 text-accent">
                  <i className="ri-school-line text-lg" aria-hidden />
                </span>
                <div>
                  <p className="text-xs font-bold uppercase tracking-wider text-accent">Where IOP helps</p>
                  <p className="mt-1 text-sm leading-6 text-white/70">
                    <AutoLinkedText>{"Exposure ladders, morning coping scripts, caregiver coaching, and school coordination — so teens\n                    approach school instead of escaping it."}</AutoLinkedText>
                  </p>
                </div>
              </div>
              <a
      ...
      <section className="relative overflow-hidden bg-dark px-6 py-16 lg:px-10 lg:py-20">
        ...
        <div className={`relative ${CONTAINER}`}>
          <div className="mb-10 max-w-2xl">
            <p className="mb-3 text-[11px] font-bold uppercase tracking-[0.3em] text-accent">School re-entry</p>
            <h2 className="text-3xl font-bold text-white md:text-4xl lg:text-[2.75rem]" style={{ fontFamily: "var(--font-heebo)" }}>
              A gradual return plan — paced, never forced
            </h2>
            <p className="mt-4 text-sm leading-8 text-white/50">
              <AutoLinkedText>{"Teens do not start with a full school day on day one. Clinicians build individualized exposure hierarchies\n              with homework between IOP sessions — celebrating effort over perfect attendance."}</AutoLinkedText>
            </p>
          </div>
          ...
          <p className="mt-5 text-xs leading-5 text-white/35">
            <AutoLinkedText>{"Example re-entry sequence — every teen&apos;s plan is individualized during assessment."}</AutoLinkedText>
          </p>
        </div>
      </section>
      ...
      <section className="bg-surface px-6 py-16 lg:px-10 lg:py-20">
        <div className={CONTAINER}>
          <div className="mb-10 max-w-2xl">
            <p className="mb-3 text-[11px] font-bold uppercase tracking-[0.3em] text-accent"><AutoLinkedText>{"Family & school dynamics"}</AutoLinkedText></p>
            <h2 className="text-3xl font-bold text-ink md:text-4xl" style={{ fontFamily: "var(--font-heebo)" }}>
              Aligning caregivers and schools for sustainable re-entry
            </h2>
            <p className="mt-4 text-sm leading-8 text-body">
              <AutoLinkedText>{"Accommodation often grows quietly — extra reassurance, missed days without a plan, or conflict every\n              morning. IOP family sessions address patterns that keep avoidance going."}</AutoLinkedText>
            </p>
          </div>
          <div className="space-y-3">
            {familySchoolShifts.map((row) => (
              <div
                key={row.before}
          ...
          <div className="overflow-hidden rounded-3xl bg-dark ring-1 ring-white/10">
            <div className="grid lg:grid-cols-[1fr_340px]">
              <div className="p-8 lg:p-10">
                <p className="text-[11px] font-bold uppercase tracking-[0.3em] text-accent">Return to school</p>
                <h2 className="mt-4 text-3xl font-bold text-white md:text-4xl" style={{ fontFamily: "var(--font-heebo)" }}>
                  Clinical treatment and re-entry — in the same program
                </h2>
                <p className="mt-4 text-sm leading-8 text-white/55">
                  <AutoLinkedText>{"Virtual IOP meets teens where they are during active avoidance — treating anxiety or mood while\n                  building attendance step by step. We coordinate with schools when families authorize it."}</AutoLinkedText>
                </p>
                ...
                <div className="mt-8 flex flex-wrap gap-3">
                  ...
                  <AmhButton href="/online-cognitive-behavioral-therapy" variant="darkSecondary" icon="ri-arrow-right-line">
                    About CBT
                  </AmhButton>
                </div>
              </div>
              <div className="border-t border-white/10 bg-white/[0.03] p-8 lg:border-l lg:border-t-0 lg:p-10">
                ...
                <div className="mt-5 space-y-3">
                  {[
                    "Stomachaches or headaches only on school days",
                    "Begging to stay home or leaving school early",
                    "Sleep disruption on Sunday nights",
                    "Grades dropping after a period of good performance",
                  ].map((sign) => (
      ...
      <section className="bg-white px-6 py-16 lg:px-10 lg:py-20">
        <div className={CONTAINER}>
          <div className="max-w-2xl">
            <p className="text-[11px] font-bold uppercase tracking-[0.3em] text-accent">Underlying drivers</p>
            <h2 className="mt-4 text-3xl font-bold text-ink md:text-4xl" style={{ fontFamily: "var(--font-heebo)" }}>
              Treat the condition — not just the absences
            </h2>
            <p className="mt-4 text-sm leading-8 text-body">
              <AutoLinkedText>{"School avoidance is usually a symptom of something treatable. IOP addresses the full clinical picture —\n              not attendance alone."}</AutoLinkedText>
            </p>
          </div>
          <div className="mt-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {underlyingDrivers.map((item) => (
              <Link
                key={item.label}
                href={item.path}
