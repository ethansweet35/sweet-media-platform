...
const SITE_URL = process.env.NEXT_PUBLIC_SITE_URL || "https://rizeoc.com";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  // If not a blog post slug, don't render metadata (notFound will handle it)
  if (!row) {
    return {
      title: "Not Found | Rize OC",
      robots: { index: false },
    };
  }

  const titleBase = metaString(row.title) || slugToFallbackTitle(slug);
  const title = `${titleBase} | Rize OC`;
  const description =
    metaString(row.meta_description).trim() ||
    metaString(row.excerpt).trim() ||
    "Mental health, addiction recovery, and trauma-informed care resources from Rize OC.";
