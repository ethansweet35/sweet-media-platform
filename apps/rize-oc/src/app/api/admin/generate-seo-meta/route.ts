...
function buildPrompt(body: GenerateSeoMetaRequest): string {
  ...
  if (isPost) {
    ...
    return `You are an expert SEO copywriter for a company (Rize OC). Generate an optimized meta description for a blog post.
...
- Return ONLY valid JSON, no markdown fences: {"seo_title":null,"meta_description":"..."}`;
  ...
  return `You are an expert SEO copywriter for a company (Rize OC). Generate an SEO title and meta description for the following page.
