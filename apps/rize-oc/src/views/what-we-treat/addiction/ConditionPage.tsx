/**
 * Reusable template for addiction sub-pages.
 *
...
export default function ConditionPage({ data }: { data: ConditionData }) {
  const {
        ...
        <div className="relative z-10 mx-auto w-full max-w-[1300px] px-6 lg:px-10 py-20 lg:py-0 grid lg:grid-cols-[1fr_400px] gap-10 xl:gap-16 items-center min-h-[92vh]">
          ...
          <div className="flex flex-col justify-center">
            ...
            <div className="mt-10 flex flex-wrap items-center gap-x-6 gap-y-3">
              {[
                { icon: "ri-checkbox-circle-line", text: "Medically Supervised" },
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white border border-warm p-8">
              <div className="flex items-center gap-3 mb-6">
                <IconCircle icon="ri-heart-pulse-line" variant="accent-subtle" size="sm" />
                <h3 className="font-[family-name:var(--font-display)] text-[22px] font-normal text-ink">Physical Signs</h3>
              </div>
              <div className="flex flex-col gap-0">
                {physicalSymptoms.map(({ text }, i) => (
                  <div
                    key={i}
                    className="flex items-start gap-3 py-3 border-b border-warm last:border-0"
                  >
            ...
            <div className="bg-white border border-warm p-8">
              <div className="flex items-center gap-3 mb-6">
                <IconCircle icon="ri-mental-health-line" variant="muted-subtle" size="sm" />
                <h3 className="font-[family-name:var(--font-display)] text-[22px] font-normal text-ink">Behavioural Signs</h3>
              </div>
              <div className="flex flex-col gap-0">
                {behavioralSymptoms.map(({ text }, i) => (
                  <div
                    key={i}
                    className="flex items-start gap-3 py-3 border-b border-warm last:border-0"
                  >
          ...
          <div className="grid sm:grid-cols-3 gap-px bg-white/5">
            {consequences.map(({ icon, title, desc }, i) => (
              <div key={title} className="relative bg-ink p-8 flex flex-col gap-5">
                <span
                  className="absolute top-6 right-6 font-[family-name:var(--font-display)] text-[64px] font-normal text-white/5 leading-none select-none"
                  aria-hidden
                >
                  {String(i + 1).padStart(2, "0")}
                </span>
        ...
        <SectionWrapper>
          ...
          <div className="flex flex-col gap-0">
            {treatmentSteps.map(({ title, desc, detail }, i) => (
              <div
                key={title}
                className={`grid lg:grid-cols-[80px_1fr_1fr] gap-6 lg:gap-12 py-8 border-b border-warm last:border-0 items-start`}
              >
                {/* Step number */}
                <div className="flex items-center gap-4 lg:flex-col lg:items-start">
                  ...
                  {i < treatmentSteps.length - 1 && (
                    <div className="hidden lg:block h-8 w-px bg-warm mt-2" />
                  )}
                </div>
          ...
          <div className="mt-8 bg-ink px-8 py-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <a
              href="tel:9494612620"
              className="shrink-0 flex items-center gap-2 text-accent text-[11px] font-medium uppercase tracking-[0.2em] hover:gap-3 transition-all whitespace-nowrap"
            >
