...
export default function ContactHeroSection() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    // Handle checkboxes: "seeking_help_for" may have multiple values
    const seekingHelpValues = formData.getAll("seeking_help_for");
    params.delete("seeking_help_for");
    if (seekingHelpValues.length > 0) {
      params.append("seeking_help_for", seekingHelpValues.join(", "));
    }

    try {
      const response = await fetch("https://readdy.ai/api/form/d851i7ic1es11hk811mg", {
        method: "POST",
        headers: {
