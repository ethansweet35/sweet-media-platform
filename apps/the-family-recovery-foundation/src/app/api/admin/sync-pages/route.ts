...
function resolveRoutePath(file: string): string | null {
  ...
  for (const s of segs) {
    if (s.startsWith("@") || /^\([^)]+\)$/.test(s) || s.includes("[") || s === "admin" || s === "api") {
      return null;
    }
  }
...
export async function POST() {
  ...
  if (!supabaseUrl || !serviceKey) {
    return NextResponse.json(
      {
        error:
          "SUPABASE_SERVICE_ROLE_KEY is not set in Vercel environment variables. Add it under Settings -> Environment Variables.",
      },
      { status: 500 },
    );
  }
