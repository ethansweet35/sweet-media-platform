...
const SITE_URL = process.env.NEXT_PUBLIC_SITE_URL || "https://tfrfoundation.org";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = `${titleBase} | The Family Recovery Foundation`;
  const description =
    metaString(row?.meta_description).trim() ||
    metaString(row?.excerpt).trim() ||
    "Mental health, addiction recovery, and trauma-informed care resources from The Family Recovery Foundation.";
  ...
  return {
    ...
    openGraph: {
      ...
      title,
      description,
      siteName: "The Family Recovery Foundation",
    ...
    <>
      ...
      {post.content_editor_id ? (
        <OptimizationStatusBanner
          ...
          brandName="The Family Recovery Foundation"
        />
      ) : null}
