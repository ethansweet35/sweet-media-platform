...
const TO = 'admissions@innerpeakcolorado.com';
const SUBJECT = 'New Contact Form Submission - Inner Peak Colorado';

function row(label: string, value: string | undefined) {
  if (!value) return '';
  return `<tr>
    <td style="padding:8px 12px;font-weight:600;color:#2C3B2E;background:#F0ECE1;width:160px;vertical-align:top;">${label}</td>
...
<body style="font-family:'DM Sans',system-ui,sans-serif;background:#FAF8F5;margin:0;padding:32px;">
  <div style="max-width:600px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(44,59,46,0.08);">
    <div style="background:#2C3B2E;padding:28px 32px;">
      <h1 style="margin:0;color:#FAF8F5;font-size:20px;font-weight:600;">Inner Peak Colorado</h1>
    ...
    <div style="padding:16px 32px;background:#F0ECE1;font-size:12px;color:#6B7D67;border-top:1px solid #e0dbd0;">
      Submitted via innerpeakcolorado.com
    </div>
  </div>
</body>
</html>`;
...
function buildText(fields: Record<string, string | undefined>) {
  return [
    `New Contact Form Submission — Inner Peak Colorado`,
    ``,
...
export async function POST(req: NextRequest) {
  ...
  const { error } = await resend.emails.send({
    from: 'Inner Peak Colorado <no-reply@innerpeakcolorado.com>',
    to: [TO],
    replyTo,
    subject: SUBJECT,
