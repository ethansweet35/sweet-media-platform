    ...
    <section className="relative w-full overflow-hidden bg-[#2C3B2E]">
      ...
      <div className="relative max-w-7xl mx-auto px-8 md:px-16 pt-36 pb-16 md:pt-44 md:pb-20">
        <div className="flex items-center gap-2 mb-8">
          <Link
            href="/blog"
            className="flex items-center gap-2 text-[10px] uppercase tracking-[0.25em] text-[#F0ECE1]/45 hover:text-[#F0ECE1]/80 transition-colors"
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_420px] gap-12 lg:gap-16 items-end">
          <div>
            <span className="inline-flex items-center rounded-full border border-[#F0ECE1]/15 bg-[#F0ECE1]/10 px-4 py-2 text-[10px] uppercase tracking-[0.25em] text-[#DDA15E]">
              {post.category}
            </span>
            ...
            {post.excerpt ? (
              <p className="text-[#F0ECE1]/70 font-light text-base md:text-lg leading-[1.9] max-w-3xl">
                {post.excerpt}
              </p>
            ) : null}

            <div className="mt-9 flex flex-wrap items-center gap-4 md:gap-6 border-t border-[#F0ECE1]/10 pt-6">
              <div className="flex items-center gap-3">
                <div className="relative w-11 h-11 rounded-full overflow-hidden bg-[#F0ECE1]/10 flex-shrink-0">
                  {post.authorPhoto ? (
                    <Image
                ...
                <div>
                  <p className="text-sm font-medium text-[#FAF8F5] leading-tight">
                    {post.author || "Inner Peak Colorado"}
                  </p>
                  <p className="text-[11px] text-[#F0ECE1]/45">
                    {post.authorRole || "Clinical Editorial Team"}
                  </p>
                </div>
              </div>

              <div className="hidden md:block w-px h-9 bg-[#F0ECE1]/10" />

              <div className="flex items-center gap-2 text-[#F0ECE1]/45">
          ...
          {post.image ? (
            <div className="relative aspect-[3/2] w-full max-w-[520px] lg:ml-auto rounded-[2rem_0.75rem_2rem_0.75rem] overflow-hidden border border-[#F0ECE1]/10 bg-[#F0ECE1]/5 shadow-2xl shadow-black/20">
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-t from-[#2C3B2E]/40 via-transparent to-transparent" />
            </div>
          ) : null}
        </div>
        ...
        {post.tags?.length ? (
          <div className="mt-8 flex flex-wrap gap-2">
            {post.tags.map((tag) => (
              <span
                key={tag}
                className="text-[9px] tracking-widest uppercase text-[#F0ECE1]/40 bg-[#F0ECE1]/5 border border-[#F0ECE1]/10 px-3 py-1.5 rounded-full"
              >
                {tag}
              </span>
            ))}
