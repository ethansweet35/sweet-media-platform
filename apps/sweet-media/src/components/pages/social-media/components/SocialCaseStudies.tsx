...
function MiniSparkline({ pts, animate }: { pts: number[]; animate: boolean }) {
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${w} ${h + 12}`} className="overflow-visible">
      <defs>
        <linearGradient id="smSparkGrad" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="white" stopOpacity="0.18" />
          <stop offset="100%" stopColor="white" stopOpacity="0" />
        </linearGradient>
        <clipPath id="smSparkClip">
      ...
      <path d={fillD} fill="url(#smSparkGrad)" clipPath="url(#smSparkClip)" />
      <path d={d} fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" clipPath="url(#smSparkClip)" />
      {labelPairs.map(([idx]) => (
      ...
      <div className="max-w-screen-xl mx-auto">
        <div className="mb-14">
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
            <h2 className="text-4xl md:text-5xl font-bold text-white leading-tight text-center lg:text-left" style={{ fontFamily: "'Playfair Display', serif" }}>
              Real Social Campaigns,
              <br /><em className="font-light italic" style={{ color: "#7B9FD4" }}>Real Engagement.</em>
            </h2>
            <p className="text-white/50 text-sm leading-relaxed max-w-md font-light text-center lg:text-right mx-auto lg:mx-0">
              Every case below is a real behavioral health client. Real numbers, real timelines, real community growth.
            </p>
          </div>
        </div>
        ...
        <div className="border-t border-white/10 mt-14 pt-10 grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
          {[
            ...
            { val: "$115", label: "Avg CPA at Scale" },
