...
import { RESULTS_STATS_GRID, RESULTS_STATS_PROOF } from "@/views/results/resultsContentDefaults";
...
const stats = RESULTS_STATS_GRID;
...
export interface ResultsStatsProps {
  ...
  statValues?: ReactNode[];
  statLabels?: ReactNode[];
  statSubs?: ReactNode[];
  proofValues?: ReactNode[];
  proofLabels?: ReactNode[];
}
      ...
      <div className="relative max-w-screen-xl mx-auto">
        ...
        <div className={`mb-14 transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
          <div className="flex items-center justify-center lg:justify-start gap-3 mb-5">
            ...
            <span className="text-[9px] tracking-[0.45em] uppercase text-white/40 font-medium">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-end">
            <div>
              <h2 className="text-4xl md:text-5xl font-bold text-white leading-tight mb-4 text-center lg:text-left" style={{ fontFamily: "'Playfair Display', serif" }}>
                {headlineBold ?? "Averages Across"}
                <br />
                <em className="font-light italic" style={{ color: "#7B9FD4" }}>
                  {headlineItalic ?? "All Active Accounts."}
                </em>
              </h2>
              <p className="text-white/60 text-sm leading-relaxed max-w-md font-light text-center lg:text-left mx-auto lg:mx-0">
                {body ?? "These aren't our best results — they're our typical results. Every number below is the median across our entire client base over the last 12 months."}
              </p>
            </div>
        ...
        <div className={`grid grid-cols-2 md:grid-cols-3 gap-0 border border-white/20 rounded-2xl overflow-hidden transition-all duration-700 delay-100 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
          {stats.map((s, i) => (
            ...
            >
              ...
              <div className="text-[10px] text-white/45 tracking-wide hidden sm:block">
                {statSubs?.[i] ?? s.sub}
              </div>
            </div>
          ))}
