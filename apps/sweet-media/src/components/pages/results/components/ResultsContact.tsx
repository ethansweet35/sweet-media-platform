...
export default function ResultsContact() {
  ...
  return (
    <section id="results-contact" className="w-full bg-[#0A1F44] py-[60px] md:py-[100px] px-4 md:px-6 overflow-hidden">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="text-center lg:text-left">
            ...
            <h2 className="text-4xl md:text-5xl font-bold text-white leading-tight mb-6" style={{ fontFamily: "'Playfair Display', serif" }}>
              Get Your Free
              <br /><em className="font-light italic text-white/70">Performance Audit.</em>
            </h2>
            <p className="text-white/50 text-sm leading-relaxed mb-10 max-w-md mx-auto lg:mx-0">
              We'll analyze your current marketing performance, benchmark you against similar behavioral health facilities, and show you exactly where the biggest growth opportunities are — no pitch, no commitment.
            </p>

            <div className="flex flex-col gap-4 mb-12">
              {[
                { icon: "ri-bar-chart-line", title: "Full Performance Audit", desc: "Current traffic, leads, cost-per-admission, and conversion rates — all benchmarked against industry standards." },
                { icon: "ri-search-line", title: "Competitive Gap Analysis", desc: "See exactly what your top 3 competitors are doing that you're not — and where they're vulnerable." },
                { icon: "ri-file-list-3-line", title: "90-Day Growth Roadmap", desc: "A prioritized action plan with projected outcomes, delivered within 5 business days." },
                { icon: "ri-shield-check-line", title: "HIPAA-Aware Review", desc: "Every recommendation is reviewed for compliance and patient privacy best practices." },
              ].map((item) => (
          ...
          <div className="bg-white/[0.06] border border-white/12 rounded-2xl overflow-hidden backdrop-blur-sm">
            <div className="px-7 pt-8 pb-6 border-b border-white/10">
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="text-[9px] tracking-[0.4em] uppercase text-white/45 font-medium">Free Performance Audit</span>
              </div>
              <h3 className="text-2xl font-bold text-white mb-2" style={{ fontFamily: "'Playfair Display', serif" }}>
                Let's See Where You Stand.
              </h3>
              <p className="text-white/45 text-xs leading-relaxed font-light">
                Tell us about your facility and we'll put together a complete performance audit — free, no strings.
              </p>
            </div>
              ...
              {submitted ? (
                <div className="py-10 flex flex-col items-center text-center gap-4">
                  ...
                  <p className="text-white/45 text-xs leading-relaxed max-w-[260px] font-light">
                    Our team will review your current performance and deliver your audit within 5 business days.
                  </p>
                </div>
              ) : (
                <form id="results-audit-form" onSubmit={handleSubmit} className="flex flex-col gap-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[9px] tracking-[0.3em] uppercase text-white/45 font-medium">Full Name</label>
                      <input type="text" name="name" required placeholder="Jane Smith" value={formState.name} onChange={(e) => setFormData({ ...formState, name: e.target.value })} className="bg-white/10 border border-white/15 text-white text-sm px-4 py-3 rounded-xl placeholder-white/25 focus:outline-none focus:border-white/40 transition-all font-light" />
                    </div>
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[9px] tracking-[0.3em] uppercase text-white/45 font-medium">Phone</label>
                      <input type="tel" name="phone" placeholder="(714) 300-5115" value={formState.phone} onChange={(e) => setFormData({ ...formState, phone: e.target.value })} className="bg-white/10 border border-white/15 text-white text-sm px-4 py-3 rounded-xl placeholder-white/25 focus:outline-none focus:border-white/40 transition-all font-light" />
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <select name="program" value={formState.program} onChange={(e) => setFormData({ ...formState, program: e.target.value })} className="bg-white/10 border border-white/15 text-white/80 text-sm px-4 py-3 rounded-xl focus:outline-none focus:border-white/40 transition-all font-light appearance-none cursor-pointer">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <textarea name="message" placeholder="Your website URL and any notes about your current marketing performance..." rows={3} maxLength={500} value={formState.message} onChange={(e) => { setFormData({ ...formState, message: e.target.value }); setCharCount(e.target.value.length); }} className="bg-white/10 border border-white/15 text-white text-sm px-4 py-3 rounded-xl placeholder-white/25 focus:outline-none focus:border-white/40 transition-all font-light resize-none" />
                  ...
                  {submitError && (
                    ...
                    <button type="submit" disabled={submitting || charCount > 500} className="mt-1 w-full bg-white text-[#0A1F44] text-[11px] tracking-[0.15em] uppercase font-bold py-4 rounded-xl hover:bg-white/90 active:scale-[0.99] transition-all duration-200 cursor-pointer whitespace-nowrap disabled:opacity-50 px-4">
                    {submitting ? "Sending..." : "Request My Free Audit →"}
                  </button>

                  <p className="text-white/25 text-[10px] text-center leading-relaxed">No commitment · Delivered in 5 business days · 100% free</p>
                </form>
              )}
            </div>
          </div>
