...
const pills = ["Google Ads", "Meta Ads", "TV & Streaming", "Performance Max", "Remarketing", "Creative Production"];

export default function PaidMediaHero() {
  const [visible, setVisible] = useState(false);
  const lineRef = useRef<HTMLDivElement>(null);
  const [formState, setFormData] = useState({ name: "", email: "", phone: "", program: "", budget: "", message: "" });
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      rafId = requestAnimationFrame(() => {
        ...
        hero.style.setProperty("--orb3-y", `${scrollY * 0.52}px`);
  ...
  return (
    <section ref={heroRef} className="relative w-full min-h-screen flex items-center overflow-hidden" style={{ background: "#0A1F44" }}>
      ...
      <div className="absolute inset-0 pointer-events-none" style={{
        background: "radial-gradient(ellipse 80% 60% at 40% 40%, rgba(123,159,212,0.28) 0%, transparent 70%), radial-gradient(ellipse 50% 40% at 80% 80%, rgba(180,210,255,0.1) 0%, transparent 60%)",
        animation: "pmSweep 12s ease-in-out infinite alternate",
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute rounded-full" style={{
          width: "580px", height: "580px", top: "-160px", left: "-100px",
          background: "radial-gradient(circle, rgba(123,159,212,0.28) 0%, transparent 70%)",
          animation: "pmOrb1 18s ease-in-out infinite alternate",
        ...
        <div className="absolute rounded-full" style={{
          ...
          animation: "pmOrb2 14s ease-in-out infinite alternate",
          transform: "translateY(var(--orb2-y, 0px))",
          willChange: "transform",
        }} />
        <div className="absolute rounded-full" style={{
          width: "240px", height: "240px", top: "40%", right: "15%",
          background: "radial-gradient(circle, rgba(123,159,212,0.22) 0%, transparent 70%)",
          animation: "pmOrb3 20s ease-in-out infinite alternate",
          transform: "translateY(var(--orb3-y, 0px))",
          willChange: "transform",
        }} />
      </div>
      ...
      <div className="absolute inset-0 pointer-events-none">
        <svg className="absolute inset-0 w-full h-full opacity-[0.055]" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <pattern id="pmDots" x="0" y="0" width="40" height="40" patternUnits="userSpaceOnUse">
              <circle cx="1" cy="1" r="1" fill="white" />
            </pattern>
          </defs>
          <rect width="100%" height="100%" fill="url(#pmDots)" />
        </svg>
      </div>
      ...
      <div className="absolute inset-0 pointer-events-none" style={{
        ...
        animation: "pmBeam 8s ease-in-out infinite alternate",
      }} />
      ...
      <div className="relative z-10 w-full max-w-screen-xl mx-auto px-5 sm:px-8 md:px-12 lg:px-20 pt-32 pb-20">
        ...
        <div className="flex items-center gap-1.5 mb-10 justify-center lg:justify-start">
          ...
          <Link href="/services" className="text-[7px] md:text-[9px] tracking-[0.15em] md:tracking-[0.3em] uppercase text-white/35 hover:text-white/70 transition-colors cursor-pointer">Services</Link>
          <span className="text-white/20 text-[9px]">/</span>
          <span className="text-[7px] md:text-[9px] tracking-[0.15em] md:tracking-[0.3em] uppercase text-white/60">Paid Media</span>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-12 xl:gap-20 items-center">
          ...
          <div className="flex-1 min-w-0">
            <div className={`flex items-center gap-3 mb-8 transition-all duration-700 justify-center lg:justify-start ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
              ...
              <span className="text-[9px] tracking-[0.4em] uppercase text-white/50 font-medium">Paid Advertising</span>
            </div>
            ...
            <h1 className={`leading-[1.0] mb-6 transition-all duration-700 delay-100 text-center lg:text-left ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`} style={{ fontFamily: "'Playfair Display', serif" }}>
              <span className="block text-[42px] sm:text-[54px] md:text-[68px] font-light italic text-white/70">Paid Media For</span>
            ...
            <p className={`text-white/55 text-sm md:text-base leading-relaxed mb-10 max-w-xl font-light transition-all duration-700 delay-150 text-center lg:text-left mx-auto lg:mx-0 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
              We run Google Ads, Meta Ads, and connected TV campaigns exclusively for behavioral health facilities — with complete transparency on cost-per-lead, ROAS, and admissions impact. No guesswork, no black-box reporting.
            </p>
            ...
            <div className={`flex flex-wrap items-center gap-4 transition-all duration-700 delay-300 justify-center lg:justify-start ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
              <a href="#paid-contact" className="inline-flex items-center gap-2.5 bg-white text-[#0A1F44] text-[11px] tracking-[0.2em] uppercase font-bold px-7 py-4 rounded-full hover:bg-white/90 transition-colors cursor-pointer whitespace-nowrap">
                Get My Free Media Audit
                <i className="ri-arrow-right-line text-sm"></i>
              </a>
              <a href="#paid-google" className="hidden sm:inline-flex items-center gap-2 text-[11px] tracking-[0.2em] uppercase font-medium text-white/40 hover:text-white transition-colors cursor-pointer whitespace-nowrap">
                See Our Channels
          ...
          <div className={`w-full lg:w-[420px] xl:w-[460px] flex-shrink-0 transition-all duration-700 delay-200 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
            <div className="bg-white/[0.07] border border-white/15 rounded-2xl overflow-hidden backdrop-blur-sm">
              <div className="px-7 pt-7 pb-6 border-b border-white/10">
                ...
                <p className="text-white/45 text-xs leading-relaxed font-light">We&apos;ll show you exactly where budget is leaking — free, no commitment.</p>
              </div>
              ...
              <div className="px-7 py-6">
                {submitted ? (
                  <div className="py-10 flex flex-col items-center text-center gap-4">
                    ...
                    <p className="text-white/50 text-xs leading-relaxed max-w-[260px] font-light">Our paid media team will review your accounts and deliver your audit within 5 business days.</p>
                  </div>
                ) : (
                  <form id="paid-media-hero-form" onSubmit={handleSubmit} className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[9px] tracking-[0.3em] uppercase text-white/45 font-medium">Monthly Ad Budget</label>
                      <select name="budget" value={formState.budget} onChange={(e) => setFormData({ ...formState, budget: e.target.value })} className="bg-white/10 border border-white/15 text-white/80 text-sm px-3.5 py-2.5 rounded-xl focus:outline-none focus:border-white/40 transition-all font-light appearance-none cursor-pointer">
                        <option value="" className="text-black">Select current budget...</option>
                        <option value="Under $5K" className="text-black">Under $5K / month</option>
                        <option value="$5K–$15K" className="text-black">$5K–$15K / month</option>
                        <option value="$15K–$30K" className="text-black">$15K–$30K / month</option>
                        <option value="$30K–$75K" className="text-black">$30K–$75K / month</option>
                        <option value="$75K+" className="text-black">$75K+ / month</option>
                      </select>
                    </div>
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[9px] tracking-[0.3em] uppercase text-white/45 font-medium">Website URL or Notes</label>
                      <textarea name="message" placeholder="Your website URL and any notes about current campaigns..." rows={3} maxLength={500} value={formState.message} onChange={(e) => { setFormData({ ...formState, message: e.target.value }); setCharCount(e.target.value.length); }} className="bg-white/10 border border-white/15 text-white text-sm px-3.5 py-2.5 rounded-xl placeholder-white/25 focus:outline-none focus:border-white/40 transition-all font-light resize-none" />
                    ...
                    <button type="submit" disabled={submitting || charCount > 500} className="w-full bg-white text-[#0A1F44] text-[11px] tracking-[0.15em] uppercase font-bold py-3.5 rounded-xl hover:bg-white/90 active:scale-[0.99] transition-all duration-200 cursor-pointer whitespace-nowrap disabled:opacity-50 px-4">
                      {submitting ? "Sending..." : "Request My Free Media Audit →"}
                    </button>
                    <p className="text-white/30 text-[10px] text-center leading-relaxed">No commitment · Delivered in 5 business days · 100% free</p>
                  </form>
                )}
              </div>
            </div>
