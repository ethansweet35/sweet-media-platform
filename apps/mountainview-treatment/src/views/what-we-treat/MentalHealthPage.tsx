...
export type MentalHealthPageProps = {
  ...
  symptoms: { label: string; desc: string }[];
  // Types / Specifiers (optional — shown between symptoms and treatment)
...
const LOC_ITEMS = [
  { num: "01", title: "Partial Hospitalization Program (PHP)", href: "/levels-of-care/partial-hospitalization-program/", body: "Intensive structured programming 5–6 hours per day — the most comprehensive level of outpatient care, ideal for acute or complex presentations." },
  { num: "02", title: "Intensive Outpatient Program (IOP)", href: "/levels-of-care/intensive-outpatient-program/", body: "Approximately 9–12 hours of structured weekly programming, designed to support stabilization and skill-building alongside daily responsibilities." },
  { num: "03", title: "Outpatient Program (OP)", href: "/levels-of-care/outpatient-program/", body: "Flexible individual and group therapy on a schedule that fits your life — essential for long-term maintenance and relapse prevention." },
];

export default function MentalHealthPage(p: MentalHealthPageProps) {
  return (
    <>
      {/* ── Hero ── */}
      <section className="relative isolate flex min-h-[85vh] flex-col justify-end overflow-hidden text-white lg:min-h-[760px]">
        <Image src={p.heroImage} alt={p.heroAlt} fill priority sizes="100vw" className="-z-20 object-cover object-center" />
        <div
          aria-hidden="true"
        ...
        <div className="mx-auto max-w-[1280px] px-6 py-20 lg:px-12 lg:py-28">
          <div className="grid items-end gap-8 lg:grid-cols-[200px_1fr]">
            <p className="text-[11px] font-semibold uppercase tracking-[0.28em] text-[var(--mvt-muted)] lg:pb-2">
              Understanding<br />the Condition
            </p>
      ...
      <section className="bg-[var(--mvt-cream)] text-[var(--mvt-ink)]">
        <div className="mx-auto max-w-[1280px] px-6 py-20 lg:px-12 lg:py-24">
          ...
          <p className="mt-5 max-w-2xl text-[15px] leading-7 text-[var(--mvt-text)]">{p.symptomsIntro}</p>
          <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {p.symptoms.map((s, i) => (
              <div key={s.label} className="flex items-start gap-5 border border-black/8 bg-white p-7">
                <span className="shrink-0 font-heading text-[32px] leading-none text-[var(--mvt-teal)]/20">
                  {String(i + 1).padStart(2, "0")}
                </span>
                <div>
                  <p className="text-[14px] font-semibold text-[var(--mvt-ink)]">{s.label}</p>
                  <p className="mt-1.5 text-[13px] leading-[1.6] text-[var(--mvt-text)]">{s.desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* ── Types / Specifiers (optional) ── */}
      {p.types && p.typesHeadline && (
        <section className="bg-[var(--mvt-forest-deep)] text-white">
          <div className="mx-auto max-w-[1280px] px-6 py-20 lg:px-12 lg:py-24">
            ...
            {p.typesIntro && (
              <p className="mt-5 max-w-2xl text-[15px] leading-[1.75] text-white/70">{p.typesIntro}</p>
            )}
            <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {p.types.map((t) => (
                <div key={t.title} className="border border-white/10 bg-white/5 p-7">
                  <p className="text-[14px] font-semibold text-white">{t.title}</p>
                  <p className="mt-3 text-[13px] leading-[1.65] text-white/70">{t.body}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* ── Treatment Approach ── */}
      <section className={`${p.types ? "bg-white" : "bg-[var(--mvt-forest-deep)]"} text-${p.types ? "[var(--mvt-ink)]" : "white"}`}>
        <div className="mx-auto max-w-[1280px] px-6 py-20 lg:px-12 lg:py-28">
          <div className="grid gap-12 lg:grid-cols-[0.9fr_1.1fr] lg:gap-20 lg:items-start">
            <div className="lg:sticky lg:top-24">
              <p className={`text-[11px] font-semibold uppercase tracking-[0.30em] ${p.types ? "text-[var(--mvt-teal)]" : "text-[var(--mvt-teal-light)]"}`}>
                Our Approach
              </p>
              <h2 className={`mt-5 font-heading text-[38px] leading-[1.05] sm:text-[50px] ${p.types ? "text-[var(--mvt-ink)]" : "text-white"}`}>
                {p.approachHeadline}
              </h2>
              <p className={`mt-5 text-[15px] leading-[1.75] ${p.types ? "text-[var(--mvt-text)]" : "text-white/70"}`}>{p.approachBody}</p>
              <a
                href={SITE.phone.href}
                className={`mt-8 inline-flex items-center gap-3 px-8 py-4 text-[11px] font-semibold uppercase tracking-[0.22em] transition ${p.types ? "bg-[var(--mvt-teal)] text-white hover:bg-[var(--mvt-ink)]" : "bg-white text-[var(--mvt-ink)] hover:bg-[var(--mvt-teal)] hover:text-white"}`}
              >
            ...
            <div className="space-y-4">
              {p.approaches.map((item) => (
                <div key={item.title} className={`flex items-start gap-6 p-7 ${p.types ? "border border-black/8 bg-[var(--mvt-cream)]" : "border border-white/10 bg-white/5"}`}>
                  <span className={`mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center border ${p.types ? "border-[var(--mvt-teal)]/30 text-[var(--mvt-teal)]" : "border-[var(--mvt-teal-light)]/30 text-[var(--mvt-teal-light)]"}`}>
                    <i className={`${item.icon} text-xl`} aria-hidden="true" />
                  </span>
                  <div>
                    <p className={`text-[14px] font-semibold ${p.types ? "text-[var(--mvt-ink)]" : "text-white"}`}>{item.title}</p>
                    <p className={`mt-2 text-[13px] leading-[1.65] ${p.types ? "text-[var(--mvt-text)]" : "text-white/70"}`}>{item.body}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* ── Co-Occurring Conditions ── */}
      <section className={`${p.types ? "bg-[var(--mvt-cream)]" : "bg-[var(--mvt-cream)]"} text-[var(--mvt-ink)]`}>
        <div className="mx-auto max-w-[1280px] px-6 py-20 lg:px-12 lg:py-28">
          <div className="grid gap-12 lg:grid-cols-[1fr_1.6fr] lg:gap-20">
            <div className="lg:sticky lg:top-24 lg:self-start">
              ...
              <p className="mt-5 text-[15px] leading-[1.75] text-[var(--mvt-text)]">{p.comorbidities.intro}</p>
      ...
      <section className="bg-white text-[var(--mvt-ink)]">
        <div className="mx-auto max-w-[1280px] px-6 py-20 lg:px-12 lg:py-28">
          ...
          <div className="mt-12 space-y-4">
            {LOC_ITEMS.map((loc) => (
              <Link key={loc.num} href={loc.href} className="group flex items-start gap-6 border border-black/8 bg-white p-8 transition hover:border-[var(--mvt-teal)]/40 hover:shadow-md">
                <span className="shrink-0 font-heading text-[48px] font-light leading-none text-[var(--mvt-teal)]/20">{loc.num}</span>
      ...
      <section className="bg-[var(--mvt-cream)] text-[var(--mvt-ink)]">
        <div className="mx-auto max-w-[1280px] px-6 py-20 lg:px-12 lg:py-24">
          ...
          <div className="mt-10 flex justify-center gap-4">
            <a href={SITE.phone.href} className="inline-flex items-center gap-3 bg-[var(--mvt-teal)] px-9 py-4 text-[11px] font-semibold uppercase tracking-[0.22em] text-white transition hover:bg-[var(--mvt-ink)]">
              <i className="ri-phone-fill" aria-hidden="true" />
              {SITE.phone.display}
            </a>
            <Link href="/admissions/" className="inline-flex items-center gap-2.5 border border-[var(--mvt-ink)]/25 px-9 py-4 text-[11px] font-semibold uppercase tracking-[0.22em] text-[var(--mvt-ink)] transition hover:border-[var(--mvt-ink)]">
              Verify Insurance <i className="ri-arrow-right-line text-xs" aria-hidden="true" />
            </Link>
          </div>
        </div>
